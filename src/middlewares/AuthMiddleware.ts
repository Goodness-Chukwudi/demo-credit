...
export class AuthMiddleware extends RouterMiddleware {
  ...
  public authGuard = (req: Request, res: Response, next: NextFunction) => {
    ...
    verifyJwtToken(
      jwt,
      async (error: VerifyErrors | null, decoded?: AuthTokenPayload) => {
        try {
          if (error) {
          ...
          const loginSession = await getActiveLoginSession(
            decoded!.user,
            decoded!.loginSession
          );
  ...
  private async validateLoginSession(
