...
exports.up = async function (knex: Knex) {
  ...
  if (!hasAccountTable) {
    await knex.schema.createTable(TABLES.SettlementAccount, function (table) {
      ...
      table
        .enum("status", Object.values(ACCOUNT_STATUS))
        .notNullable()
        .defaultTo(ACCOUNT_STATUS.ACTIVE);
    });
  }
  ...
  if (!hasWalletTable) {
    await knex.schema.createTable(TABLES.Wallet, function (table) {
      ...
      table
        .enum("currency", Object.values(CURRENCY))
        .notNullable()
        .defaultTo(CURRENCY.NGN);
      table
        .integer("user_id")
        .unsigned()
        .notNullable()
      ...
      table
        .enum("status", Object.values(WALLET_STATUS))
        .notNullable()
        .defaultTo(WALLET_STATUS.ACTIVE);
  ...
  if (!hasTransactionTable) {
    await knex.schema.createTable(TABLES.Transaction, function (table) {
      ...
      table
        ...
        .inTable(TABLES.SettlementAccount);
      table
        .integer("locked_fund_id")
        .unsigned()
        .nullable()
        .references("id")
        .inTable(TABLES.LockedFund);
